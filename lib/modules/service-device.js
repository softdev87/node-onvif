...
/* ------------------------------------------------------------------
* Method: setPass(user, pass ,[callback])
* ---------------------------------------------------------------- */
OnvifServiceDevice.prototype.setPass = function(user, pass, callback) {
	let promise = new Promise((resolve, reject) => {
		let soap_body = '<SetUser xmlns="http://www.onvif.org/ver10/device/wsdl">';
		soap_body += '<User>';
		soap_body += '<tt:Username>'+user+'</tt:Username>';
		soap_body += '<tt:Password>'+pass+'</tt:Password>';
		soap_body += '<tt:UserLevel>Administrator</tt:UserLevel>';
		soap_body += '</User>';
		soap_body += '</SetUser>' ;
		let soap = this._createRequestSoap(soap_body);
		mOnvifSoap.requestCommand(this.oxaddr, 'SetPass', soap).then((result) => {
			resolve(result);
		}).catch((error) => {
			reject(error);
		});
